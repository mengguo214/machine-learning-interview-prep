# Description: 
#   This script shows Gibbs sampler for a 2-d CRP Gaussian mixture model.
#
# Details:
...
#
#   Generative model: (n is index, N is sample size)
#     z_N ~ CRP(N, alpha)
#     mu_c ~ N(mu_0, Sigma_0)
#     xn ~ N(mu_c, Sigma_0)
#   
#   Gibbs sampler: 
#     p(x_N|x_{N,-n}, x) = #C/(alpha+N-1) . likelihood(if n joins cluster C)
#     p(x_N|x_{N,-n}, x) = alpha/(alpha+N-1) . likelihood(if n starts a new cluster)
# License: GNU v3
#
# Date: October 19, 2016
#
# Authors:
#    Originally called ex7_sampler.
